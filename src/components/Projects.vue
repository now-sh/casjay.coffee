      ...
      <div class="h-100 row row-cols-1 row-cols-md-3 justify-content-center">
        <div v-for="repo in (repos as GitHubRepo[])" :key="repo.id" class="col h-100 p-2">
          <div class="card border-danger h-100">
            <img class="card-img-top rounded" :src="orgData?.avatar_url || '/logo.png'" :alt="repo.name" loading="lazy" />
            <div class="card-body">
            ...
            <div class="card-footer">
              ...
              <a :href="`https://github.com/${repo.full_name}`" class="btn btn-danger btn-outline-success card-link" target="_blank" rel="noopener noreferrer">
                View GitHub Repo
              </a>
            </div>
          </div>
...
const isErrorResponse = computed(() => {
  ...
  return !Array.isArray(repos.value) && typeof repos.value === 'object' && 'message' in repos.value;
});

// Computed property to check if we should show empty state
const showEmptyState = computed(() => !repos.value || (Array.isArray(repos.value) && repos.value.length === 0) || isErrorResponse.value);
